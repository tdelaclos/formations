...
```text
sentinel --config /etc/sentinel/sentinel.conf
sentinel --config /etc/sentinel/sentinel.conf --check-config
sentinel --version
```

Si votre version actuelle ne propose pas ces interfaces, ajoutez-les avant le packaging. Un service industrialisé doit pouvoir annoncer sa version et valider sa configuration sans démarrer.
...
ExecStart=/usr/libexec/sentinel/sentinel --config /etc/sentinel/sentinel.conf
Restart=on-failure
RestartSec=5s
...
## TP 3 — Construire et qualifier une mise à niveau

Créez une version `1.1.0` du code et ajoutez une option non obligatoire à `sentinel.conf`. Mettez à jour `Version`, remettez `Release` à `1`, puis reconstruisez.
...
```bash
sudo dnf upgrade ./sentinel-1.1.0-1.el9.noarch.rpm
rpm -q sentinel
...
stateDiagram-v2
    ...
    V1Configurée --> V11Préparée: build et tests
    V11Préparée --> V11Installée: dnf upgrade
    V11Installée --> Fusion: traiter .rpmnew
...
```bash
rpmsign --addsign ~/rpmbuild/RPMS/noarch/sentinel-1.1.0-1*.rpm
rpm -Kv ~/rpmbuild/RPMS/noarch/sentinel-1.1.0-1*.rpm
```
...
← [10.5 — Exploiter un dépôt RPM privé](10.5-exploiter-depot-rpm-prive.md) · [11.1 — Découvrir Podman](../campagne%2011/11.1-decouvrir-podman.md) →
