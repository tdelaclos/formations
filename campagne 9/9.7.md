...
server:
  ...
  bind: {{ sentinel.server.bind }}

  port: {{ sentinel.server.port }}

logging:

  directory: {{ sentinel.logging.directory }}

tls:

  enabled: {{ sentinel.tls.enabled }}

  certificate: {{ sentinel.tls.certificate }}

  private_key: {{ sentinel.tls.private_key }}
...
ExecStart={{ sentinel.install_dir }}/bin/sentinel \
    --config /etc/sentinel/sentinel.yml
