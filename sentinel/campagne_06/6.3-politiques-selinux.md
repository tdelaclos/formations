...
```mermaid
flowchart TD
    Source["Type source"] --> Rule["Règle de Type Enforcement"]
    Target["Type cible"] --> Rule
    Class["Classe et permission"] --> Rule
    Rule --> Decision{"Règle allow présente ?"}
    Decision -->|Oui| Permit["Autoriser"]
    Decision -->|Non| Deny["Refuser et journaliser"]
```
...
← [6.2 — Les contextes SELinux](6.2-contextes-selinux.md) · [6.4 — Diagnostic des refus SELinux](6.4-diagnostic-refus-selinux.md) →
