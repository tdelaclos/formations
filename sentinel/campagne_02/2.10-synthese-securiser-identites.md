...
Créez un petit laboratoire sous `/srv/sentinel-lab` sans y placer de secret réel. Il doit comporter une configuration, un répertoire de données, un journal et un script d'administration. Définissez :
...
Testez ensuite la matrice sous chaque identité. Une preuve réussie ne suffit pas : ajoutez au moins un test de refus par exigence. Collectez `getent`, `id`, `stat`, `getfacl`, `getfattr`, `sudo -l` et les journaux utiles dans un dossier de preuves sans copier de condensat de mot de passe.
