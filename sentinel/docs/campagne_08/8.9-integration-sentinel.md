...
Le découpage rend cette frontière visible : `tls_support.py` authentifie cryptographiquement le pair, puis `identity.py` extrait et autorise les SAN DNS. `web.py` enchaîne ces contrôles avant de router la requête. Ainsi, un débutant peut étudier authentification et autorisation dans deux fichiers courts au lieu de les confondre dans un gestionnaire HTTP monolithique.

Le contrôle applicatif de `src/identity.py` extrait exclusivement les entrées SAN de type DNS :
