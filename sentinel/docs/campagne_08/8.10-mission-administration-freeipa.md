...
- le déploiement du checkpoint Sentinel 0.6.0 ;
- une identité dédiée au healthcheck ;
- une liste d'identités DNS clientes autorisées.

Justifiez le rôle de chacun de ces éléments.

La mission ne consiste pas à écrire une nouvelle application. Déployez le même commit 0.6.0 que celui qualifié au chapitre 8.9, activez le mTLS et fournissez les preuves suivantes :

```text
certificat autorisé                  → HTTP 200
certificat FreeIPA non autorisé      → HTTP 403
certificat d'une autre autorité      → échec TLS
client sans certificat               → échec TLS
healthcheck avec identité dédiée     → code de retour 0
```

Renouvelez ensuite le certificat serveur dans le laboratoire. Notez son numéro de série avant et après, déclenchez l'action de prise en compte prévue et prouvez que Sentinel présente le nouveau certificat sans modification du code ni copie de clé privée dans Git.
...
- Sentinel 0.6.0 distingue un certificat authentifié d'une identité autorisée.
- Le renouvellement est effectivement présenté par le service, pas seulement marqué `MONITORING`.
