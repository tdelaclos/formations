...
- reconnaître le type, le propriétaire, le groupe et les permissions `rwx` d'un objet ;
...
- `root` porte l'UID 0 et concentre de nombreux privilèges, sans être soustrait à toutes les autres couches de sécurité.
- Le premier caractère de `ls -l` distingue fichier, répertoire, lien, périphérique, socket ou tube nommé.
...
![Récapitulatif visuel du chapitre 1.7](media/recap-1.7.png)
