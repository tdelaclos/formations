...
```mermaid
flowchart LR
    Disk["Disque virtuel"] --> ESP["Partition système EFI\namorçage UEFI"]
    Disk --> Boot["/boot\nnoyaux et initramfs"]
    Disk --> LVM["Volume physique LVM"]
    LVM --> Root["Volume logique /\nsystème XFS"]
    LVM --> Swap["swap\nselon le profil"]
```
...
- Le partitionnement automatique articule démarrage UEFI, `/boot`, LVM, racine et éventuellement swap ; il doit être vérifié après installation.
- XFS est conservé comme choix de plateforme, sans présenter ext4 comme un système de fichiers inadéquat.
...
![Récapitulatif visuel du chapitre 1.2](media/recap-1.2.png)

## Pour aller plus loin

Conservez les [notes de version AlmaLinux](https://wiki.almalinux.org/release-notes/), les instructions correspondant à la version installée et la documentation de votre hyperviseur. Les écrans d'Anaconda évoluent ; la fiche doit décrire les résultats attendus, pas seulement une suite de clics.
