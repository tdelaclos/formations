...
- `src/` contient le programme Python exécutable ;
...
- `application == "sentinel"` et `status == "ok"` ;
- la présence d'un nom d'hôte et d'une version de noyau ;
- la sortie exacte de `--version` ;
- le caractère analysable de la sortie JSON ;
- le refus du format inexistant `xml` avec un code non nul.
...
- présence de `README.md`, `pyproject.toml`, `src/sentinel.py` et `tests/` ;
...
2. programme exécutable et tests automatisés ;
3. README avec objectif, périmètre et démarrage ;
4. décision d'architecture avec invariants ;
5. `.gitignore` commenté ;
6. contrôle de structure exécutable ;
7. conventions de commit et de revue ;
8. matrice des chemins futurs issue du chapitre 1.6 ;
9. modèle d'identité issu du chapitre 1.7 ;
10. règles d'administration issues du chapitre 1.8 ;
11. référence vers la baseline acceptée au chapitre 1.9, sans recopier de données sensibles.
...
![Récapitulatif visuel du chapitre 1.10](media/recap-1.10.png)
...
← [1.9 — Mission : mettre le serveur en sécurité](1.9-premiere-mise-en-securite-serveur.md) · [2.1 — Les permissions Unix](../campagne_02/2.1-permissions-unix.md) →
