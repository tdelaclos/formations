# Chapitre 9.10 — Mission : déployer l'infrastructure Sentinel
...
Ce chapitre fournit le modèle mental et les pratiques nécessaires pour aborder **Mission : déployer l'infrastructure Sentinel** dans un socle AlmaLinux sécurisé et reproductible.
...
L'équipe a qualifié Sentinel 0.6.0 avec son intégration FreeIPA.
...
La mission finale ne crée aucune fonction Python. Elle doit prouver que le même produit, le même contrat de configuration et la même politique d'identité peuvent être reconstruits sur plusieurs serveurs.

### État attendu

Sur `sentinel01` et `sentinel02`, collectez automatiquement :

```text
sentinel --version                         → sentinel 0.6.0
rpm -q sentinel                            → même NEVRA
sha256sum de l'exécutable                  → même empreinte
--check-config                             → succès
systemctl is-active sentinel               → active
GET /ready avec certificat healthcheck     → HTTP 200
GET /api/v1/status avec agent autorisé     → HTTP 200
certificat FreeIPA non autorisé            → HTTP 403
```

Les empreintes de configuration peuvent différer lorsque les noms d'hôtes, certificats ou populations autorisées diffèrent. Cette variation doit provenir de variables d'inventaire explicites, jamais d'une correction manuelle après le playbook.

### Scénario de reconstruction

1. ajoutez une VM AlmaLinux minimale à l'inventaire ;
2. lancez le playbook depuis un état documenté ;
3. mesurez les changements du premier passage ;
4. exécutez les tests fonctionnels et de refus ;
5. relancez le playbook sans modifier les entrées ;
6. exigez `changed=0` pour les rôles stables ;
7. comparez version, empreinte du code et contrats HTTP aux deux serveurs existants.

Une idempotence parfaite ne suffit pas si le rôle reproduit une erreur. Inversement, un service fonctionnel obtenu après retouche manuelle ne constitue pas un déploiement reproductible.

### Livrables du jalon

- commit exact de Sentinel 0.6.0 ;
- inventaire et variables sans secrets ;
- rôle de déploiement et handlers ;
- rapport des premier et second passages ;
- matrice des différences attendues entre hôtes ;
- résultats des tests mTLS positifs et négatifs ;
- procédure de retour à la version précédemment qualifiée.

Le prochain changement de code aura lieu lorsque le produit sera stabilisé et empaqueté dans la campagne 10.
...
Le chapitre **Mission : déployer l'infrastructure Sentinel** établit une brique du socle de sécurité Sentinel.
...
La campagne 10 transforme le déploiement automatisé en livraison gérée par AlmaLinux. Vous y construirez le paquet RPM de Sentinel, déclarerez ses dépendances, préserverez sa configuration pendant les mises à niveau, signerez les artefacts et les publierez dans un dépôt DNF privé.

Ansible continuera d'orchestrer le déploiement, mais RPM deviendra responsable des fichiers, des versions et du cycle de vie du logiciel installé.

---

← [9.9 — Industrialiser le laboratoire](9.9-industrialiser-laboratoire.md) · [10.1 — Construire un paquet RPM](../campagne_10/10.1-construire-paquet-rpm.md) →
