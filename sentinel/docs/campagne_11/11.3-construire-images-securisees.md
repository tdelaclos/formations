...
RUN python3 -m py_compile /opt/sentinel/*.py \
    && mkdir -p /var/lib/sentinel \
    && chmod 0444 /opt/sentinel/*.py \
...
Remplacez le digest et adaptez `--healthcheck` à l'interface réellement développée. La commande doit retourner `0` lorsque le service est sain et une valeur non nulle sinon. Le motif `src/*.py` embarque le lanceur et ses modules, mais pas les tests, configurations ni caches : vérifiez le contenu de `src/` pendant la revue du build.
