...
| --- | --- | --- |
| `GET /health` | prouver que le processus traite une requête | HTTP 200 et version |
| `GET /api/v1/status` | publier le dernier diagnostic enregistré | document JSON de Sentinel 0.3.0 |
...
1. requête locale réussie ;
2. requête depuis Kali autorisée réussie ;
3. requête depuis une source non autorisée refusée ou expirée ;
4. aucune modification de `status.json` par le client HTTP, car l'API reste en lecture seule.
...
11. Le commit Sentinel 0.3.0 et les résultats des tests HTTP.
12. Les preuves croisées d'un client autorisé et d'une source refusée.
