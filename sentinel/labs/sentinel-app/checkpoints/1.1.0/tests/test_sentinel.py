"""Tests du checkpoint Sentinel 1.1.0."""
...
class SentinelTests(unittest.TestCase):
    ...
    def test_metrics_reflect_real_requests_with_bounded_labels(self) -> None:
        self.fetch("/health")
        self.fetch("/ready")
        with self.assertRaises(urllib.error.HTTPError):
            urllib.request.urlopen(self.base_url + "/users/alice", timeout=2)

        with urllib.request.urlopen(self.base_url + "/metrics", timeout=2) as response:
            metrics = response.read().decode("utf-8")

        self.assertIn(
            'sentinel_build_info{version="1.1.0",revision="unknown"} 1', metrics
        )
        self.assertIn(
            'sentinel_http_requests_total{method="GET",route="/health",code="200"} 1',
            metrics,
        )
        self.assertIn(
            'sentinel_http_requests_total{method="GET",route="unknown",code="404"} 1',
            metrics,
        )
        self.assertIn("sentinel_http_request_duration_seconds_bucket", metrics)
        self.assertIn('sentinel_dependency_up{name="state"} 1', metrics)
        self.assertNotIn("alice", metrics)
        self.assertNotIn("127.0.0.1", metrics)

    def test_prometheus_label_values_are_escaped(self) -> None:
        self.assertEqual(
            SENTINEL.escape_label_value('build\\feature"42\nnext'),
            'build\\\\feature\\"42\\nnext',
        )
